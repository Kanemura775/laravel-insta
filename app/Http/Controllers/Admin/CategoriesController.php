...
class CategoriesController extends Controller
{
    ...
    public function index()
    {
        ...
        //to get the count of the post without categories
        $uncategorized_count = 0;
        $all_posts = $this->post->all();
        foreach ($all_posts as $post) {
            if ($post->categoryPost->count() == 0) {
                $uncategorized_count++; //increament to 1 or add 1
            }
        }
    ...
    public function store(Request $request)
    {
        ...
        /**
         * input = ACAdeme
         * PART: strtolower($request->name)   => output = academe
         * strtolower() converts input to all lowercase
         * PART: ucwords()                    => output = Academe
         * ucwords() converts input's first letter to upper case
         */
        $this->category->save();
